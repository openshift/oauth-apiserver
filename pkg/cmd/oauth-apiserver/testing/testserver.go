...
func StartTestServer(t Logger, customFlags []string, storageConfig *storagebackend.Config) (result TestServer, err error) {
	...
	if err := completedServerConfig.WithOpenAPIAggregationController(preparedOAuthServer.GenericAPIServer, completedServerConfig.GenericConfig.OpenAPIConfig); err != nil {
		return result, err
	}
	if err := completedServerConfig.WithOpenAPIV3AggregationController(preparedOAuthServer.GenericAPIServer); err != nil {
		return result, err
	}
