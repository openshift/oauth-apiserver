...
import (
	...
	apiservervalidation "github.com/openshift/oauth-apiserver/pkg/externaloidc/internal/thirdparty/kubernetes/apiserver/pkg/apis/apiserver/validation"
...
func convertCELValueToStringList(val ref.Val) ([]string, error) {
	...
	case celgo.ListType(nil).TypeName():
		...
		case []string:
			for _, out := range val.Value().([]string) {
				if len(out) == 0 {
					continue
				}
